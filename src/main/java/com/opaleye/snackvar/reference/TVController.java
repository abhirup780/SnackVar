...
public class TVController implements Initializable {
	@FXML private TableView<TranscriptVariant> tableView;
	...
	/** Columns come from FXML untyped, so the element type is reasserted here. */
	@SuppressWarnings("unchecked")
	private <T> TableColumn<TranscriptVariant, T> column(int index) {
		return (TableColumn<TranscriptVariant, T>) tableView.getColumns().get(index);
	}
	...
	public void init(Vector<TranscriptVariant> tvList) {
		...
		ObservableList<TranscriptVariant> observableTvList = FXCollections.observableArrayList(tvList);

		TableColumn<TranscriptVariant, String> tcDescription = column(0);
		tcDescription.setCellValueFactory(new PropertyValueFactory<>("description"));

		TableColumn<TranscriptVariant, Number> tcSize = column(1);
		...
		// Pre-select the first transcript so 'Choose' is always meaningful.
		if(!observableTvList.isEmpty())

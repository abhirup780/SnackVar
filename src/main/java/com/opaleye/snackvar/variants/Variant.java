...
public class Variant implements Comparable<Variant>{
	...
	public int compareTo(Variant s) {
		...
		// Heterozygous indels sort first, then homozygous indels, then the rest.
	...
	protected void makeTableViewProperties() {
		...
		if(aaChange == null || aaChange.isEmpty())
			aaChange = "p.?";
		...
		if(this instanceof SNV) {
			...
			StringBuilder equivBuffer = new StringBuilder();
			...
			for(int i=0;i<equivExpressionArray.length;i++) {
				...
				if(indel.getHGVS().equals(tempHGVS)) continue;	// skip the expression already shown as the variant itself

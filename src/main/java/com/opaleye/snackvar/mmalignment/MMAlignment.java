...
public class MMAlignment {
	...
	public void firstScan (char[] s1, char[] s2) {
		...
		for(int i=1;i<=M;i++) {
			// s: the diagonal (up-left) cell; CC still holds the previous row here
			s = CC[0];

			// c: the cell one to the left
			c = 0;

			// e: the current insertion score
			e = minusLimit;
	...
	private AlignedPair diff(String A, String B, double tb, double te) {
		StringBuilder alignedBuilder1 = new StringBuilder();
		...
		if(N==0) {
			if(M>0) {
				ret = new AlignedPair(A, "-".repeat(A.length()));
				return ret;
			}
			else return new AlignedPair("", "");
		}
		else if(M==0) {
			ret = new AlignedPair("-".repeat(B.length()), B);
			return ret;
		}
		else if(M==1) {
			...
			if(cost1 < cost2) {
				if(te == 0 && tb != 0) { //insertion B -> deletion A
					alignedString1 = "-".repeat(B.length()) + A;
					alignedString2 = B + "-";
				}
				else {	// deletion A -> insertion B (either order is equivalent
						// when both ends are open, so use one form)
					alignedString1 = A + "-".repeat(B.length());
					alignedString2 = "-"+B;
				}
			}
			else {
				alignedString2 = B;
				alignedBuilder1.append("-".repeat(Integer.max(0, cost2Index-1)));
				alignedBuilder1.append(A);
				alignedBuilder1.append("-".repeat(Integer.max(0, B.length()-cost2Index)));
				alignedString1 = alignedBuilder1.toString();
			}
